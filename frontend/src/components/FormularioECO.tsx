...
export default function FormularioECO() {
    ...
    const [codigosECO, setCodigosECO] = useState<CodigoMAI[]>([]);
    ...
    const loadCatalogos = async () => {
        try {
            const [prev, proc, cod, exam, diag, pers] = await Promise.all([
                api.get('/catalogos/previsiones'),
                api.get('/catalogos/procedencias'),
                api.get('/catalogos/codigos-mai?tipo_examen=ECO'),
                api.get('/catalogos/examenes-especificos?tipo_examen=ECO'),
    ...
    const confirmarEnvio = async () => {
        ...
        try {
            const dataToSend = {
                ...
                realizado_id: formData.realizado_id || undefined,
                transcribe_id: formData.transcribe_id || undefined
            };
    ...
    if (showConfirm) {
        return (
            <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow">
                <h2 className="text-xl font-bold mb-4">Confirmar Datos del Examen ECO</h2>
            ...
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow space-y-6">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <PersonalCombobox
                        label="Realizado"
                        name="realizado_id"
                        value={formData.realizado_id}
                        onChange={(val) => setFormData(prev => ({ ...prev, realizado_id: val }))}
                        tipo="GENERAL"
                    />
                    ...
                    <PersonalCombobox
                        label="Transcribe"
                        name="transcribe_id"
                        value={formData.transcribe_id}
                        onChange={(val) => setFormData(prev => ({ ...prev, transcribe_id: val }))}
                        tipo="GENERAL"
                    />
                </div>
